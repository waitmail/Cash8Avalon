...
namespace Cash8Avalon
{
    ...
    public class PaymentSnapshot
    {
        ...
        public decimal CertificateMoney { get; set; }
        public decimal BonusMoney { get; set; }
        public decimal TotalSumAtDiscount { get; set; }
    ...
    public partial class Cash_check : Window
    {
        ...
        private bool _isPayFormOpening = false;

        public PaymentSnapshot LastPaymentSnapshot { get; set; }
        ...
        private int _productsCurrentRow = 0; // ИЗМЕНИТЬ на 0 (раньше было 1)
    ...
    UpdatePaymentInfoRowVisibility();
        ...
        private async void OnGlobalKeyDownForForm(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                // ===== ГЛАВНОЕ ИСПРАВЛЕНИЕ =====
                // Если фокус находится внутри модального дочернего окна (TovarNotFound, Pay и т.д.),
                // то родительское окно Cash_check НЕ должно реагировать на клавиши!
                if (e.Source is Visual sourceVisual)
                {
                    var sourceWindow = sourceVisual.FindAncestorOfType<Window>();
                    if (sourceWindow != null && sourceWindow != this)
                    {
                        // Событие пришло из другого (дочернего модального) окна — пропускаем
                        return;
                    }
                }
                // ===============================
        ...
        */
        ...
        private void BackupProductsData()
        {
            ...
            foreach (var product in _productsData)
            {
                var backupItem = new ProductItem
                {
                    ...
                    // Критичные флаги
                    IsSertificate = product.IsSertificate,
        ...
        private void RestoreProductsData()
        {
            ...
            foreach (var backupItem in _productsDataBackup)
            {
                var restoredItem = new ProductItem
                {
                    ...
                    IsSertificate = backupItem.IsSertificate,
                    IsFractional = backupItem.IsFractional,
                    IsMarked = backupItem.IsMarked,
                    MaxQuantity = backupItem.MaxQuantity
                };
        ...
        //    // 3. Формируем очереди потерянных маркировок
        //    var missingMarkingsQueues = new Dictionary<int, Queue<string>>();
        //    foreach (var kvp in backupMarkingsByCode)
        //    {
        //        int code = kvp.Key;
        ...
        /// <summary>
        /// Восстанавливает маркировки (DataMatrix) и штрихкоды сертификатов в DataTable после обработки акций.
        /// Строго запрещено создавать новые строки, если они не являются результатом разделения схлопнувшейся строки.
        /// </summary>
        private DataTable RestoreMarkingsAfterActions(DataTable dtProcessed, List<ProductItem> backup)
        {
            ...
            // Множество кодов товаров, которые являются сертификатами
            var certificateCodes = new HashSet<int>();
            foreach (var product in backup)
            {
                if (product.IsSertificate)
                {
                    certificateCodes.Add(product.Code);
                }
            }

            // Локальная функция для строгой валидации маркировки/штрихкода
            bool IsValidMarking(string mark, int productCode)
            {
                if (string.IsNullOrWhiteSpace(mark)) return false;
                string trimmed = mark.Trim();
                if (trimmed == "0") return false;

                // Если это сертификат, его штрихкод может быть любой длины (например, 13 символов EAN-13)
                if (certificateCodes.Contains(productCode))
                {
                    return trimmed.Length > 0;
                }

                // Для обычных товаров код маркировки (DataMatrix) не может быть короче 14 символов
                return trimmed.Length >= 14;
            }
            ...
            foreach (var product in backup)
            {
                ...
                // У сертификатов IsMarked = false, поэтому добавляем проверку IsSertificate
                bool isRelevant = product.IsMarked || product.IsSertificate;

                if (isRelevant && IsValidMarking(product.Mark, product.Code))
                {
                    if (!backupMarkingsByCode.TryGetValue(product.Code, out var list))
                    {
                        list = new List<string>();
            ...
            for (int i = dtFixed.Rows.Count - 1; i >= 0; i--)
            {
                ...
                // Если строка БЕЗ валидной маркировки (например, акция затерла её нулем)
                if (!hasValidMarking)
                {
                    if (qty == 1)
                    {
                        ...
                        MainStaticClass.WriteRecordErrorLog(
                            "Акция удалила маркировку/штрихкод из строки (qty=1)",
                            "RestoreMarkingsAfterActions",
                            this.numdoc,
                            MainStaticClass.CashDeskNumber,
                            $"Код: {code}, Восстановлена: {restoredMark}");
                    ...
                    else if (qty > 1)
                    {
                        ...
                        MainStaticClass.WriteRecordErrorLog(
                            $"Акция сгруппировала товар и удалила маркировки (qty={qty})",
                            "RestoreMarkingsAfterActions",
                            this.numdoc,
                            MainStaticClass.CashDeskNumber,
                            $"Код: {code}, Восстановлено маркировок: {unitsToExtract}");
            ...
            // 5. БЕЗОПАСНЫЙ ЛОГГИНГ: Если коды всё ещё потеряны, просто фиксируем это.
            foreach (var kvp in missingMarkingsQueues)
            {
                int code = kvp.Key;
                Queue<string> marks = kvp.Value;
                ...
                string lostMarks = string.Join(", ", marks);
                MainStaticClass.WriteRecordErrorLog(
                    $"Не удалось восстановить коды для товара {code}. В таблице нет подходящих строк для их прикрепления.",
                    "RestoreMarkingsAfterActions",
                    this.numdoc,
                    MainStaticClass.CashDeskNumber,
                    $"Потерянные коды: {lostMarks}");
            }
        ...
        //{
        //    // ═══════════════════════════════════════════════
        //    //  ГВАРД: Защита от двойного клика / повторного вызова
        //    // ═══════════════════════════════════════════════
        //    if (_isPayFormOpening)
        //    {
        //        Console.WriteLine("⚠ show_pay_form уже выполняется, пропуск повторного вызова");
        //        return;
        //    }

        //    _isPayFormOpening = true; // Закрываем замок
        ...
        private async void show_pay_form()
        {
            ...
            try
            {
                ...
                // ★★★ НОВАЯ ЗАЩИТА: Не пускаем в оплату с битым сертификатом ★★★
            ...
            {
                _isPayFormOpening = false; // Открываем замок
...
// #endif
        ...
        /// <summary>
        /// Проверяет, что все сертификаты в чеке имеют корректный штрихкод.
        /// Если сертификат добавлен без штрихкода (Mark="0" или пусто), блокирует запись в БД.
        /// </summary>
        private async Task<bool> ValidateCertificatesAsync()
        {
            foreach (var product in _productsData)
            {
                if (product.IsSertificate)
                {
                    if (string.IsNullOrWhiteSpace(product.Mark) || product.Mark.Trim() == "0")
                    {
                        // Логируем критическую ошибку для ИТ-отдела
                        MainStaticClass.WriteRecordErrorLog(
                            "Попытка записи сертификата без штрихкода (Mark='0')",
                            "ValidateCertificatesAsync",
                            numdoc,
                            MainStaticClass.CashDeskNumber,
                            $"Код товара: {product.Code}, Наименование: {product.Tovar}");

                        // Показываем ошибку кассиру
                        await MessageBoxHelper.Show(
                            "⚠ ОШИБКА ПРИ ДОБАВЛЕНИИ СЕРТИФИКАТА!\n\n" +
                            $"Сертификат '{product.Tovar}' был добавлен в чек без штрихкода.\n" +
                            "СОХРАНЕНИЕ ЧЕКА НЕВОЗМОЖНО!\n\n" +
                            "ОБРАТИТЕСЬ В ИТ ОТДЕЛ.\n" +
                            "Для продолжения работы удалите этот сертификат из чека (выделите строку и нажмите клавишу Delete).",
                            "Критическая ошибка",
                            MessageBoxButton.OK,
                            MessageBoxType.Error,
                            this);

                        // Подсвечиваем проблемную строку красным (если метод есть) или просто выделяем её
                        int errorIndex = _productsData.IndexOf(product);
                        if (errorIndex >= 0)
                        {
                            HighlightRowAsError(errorIndex); // Ваш метод подсветки ошибок
                            SelectProductRow(errorIndex);
                            _productsScrollViewer?.Focus();
                        }

                        return false; // Блокируем запись
                    }
                }
            }
                                                  ...
                                                  string sertificate_money, string its_deleted, bool sendToScreen = true)
        {
            ...
            // ★★★ НОВАЯ ЗАЩИТА: Проверка сертификатов на наличие штрихкода ★★★
            if (!await ValidateCertificatesAsync())
            {
        ...
        public List<ProductItem> CreateProductsFromDataTable(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                int code = row["tovar_code"] != DBNull.Value ? Convert.ToInt32(row["tovar_code"]) : 0;
                string mark = row["marking"] != DBNull.Value ? row["marking"].ToString() : "0";

                // Восстанавливаем флаги из резервной копии (созданной в BackupProductsData)
                ProductItem backupItem = FindBackupItem(code, mark);
                ...
                var product = new ProductItem
                {
                    ...
                    // Восстанавливаем флаги, потеря которых ломает логику
                    IsSertificate = backupItem?.IsSertificate ?? false,
                    IsFractional = backupItem?.IsFractional ?? false,
                    IsMarked = backupItem?.IsMarked ?? false,

                    // Сохраняем MaxQuantity для корректной работы ограничения при возврате
                    MaxQuantity = backupItem?.MaxQuantity ?? 0
                };
        ...
        /// <summary>
        /// Ищет товар в резервной копии по коду и маркировке/штрихкоду.
        /// Для сертификатов и маркированных товаров — точное совпадение по коду И маркировке.
        /// Для обычных товаров — совпадение по коду.
        /// </summary>
        private ProductItem FindBackupItem(int code, string mark)
        {
            // 1. Для сертификатов (mark = штрихкод сертификата) и маркированных товаров
            //    ищем точное совпадение по коду И маркировке
            if (!string.IsNullOrEmpty(mark) && mark.Trim() != "0")
            {
                var item = _productsDataBackup.FirstOrDefault(p =>
                    p.Code == code && p.Mark == mark);
                if (item != null) return item;
            }

            // 2. Если не нашли по маркировке — ищем только по коду
            //    (для обычных товаров с mark = "0")
            return _productsDataBackup.FirstOrDefault(p => p.Code == code);
        }
        ...
        public async Task FindBarcodeOrCodeInTovarNewAsync(string barcode, string marking_code)
        {
            try
            {
                ...
                if (productData.isCertificate())
                {
                    // 1. Локальная проверка (не продавали ли мы его сегодня на этой кассе)
                    if (!await check_sertificate_for_sales(barcode))
                    {
                        return;
                    }
                    ...
                    bool? isActive = await MainStaticClass.CheckCertificateStatusAsync(barcode, false, this, numdoc);
                    ...
                    if (isActive == null)
                    {
                        // Произошла ошибка связи/сервера. Метод уже показал MessageBox, просто прерываем продажу.
                        return;
                    }

                    // Если isActive == false, значит сертификат не активен. Это нормально, продолжаем продажу!
                }
                ...
                // ✅ ИСПРАВЛЕНИЕ: Формируем Mark до создания объекта
                string finalMark = "0";
                if (productData.isCertificate())
                {
                ...
                var productItem = new ProductItem
                {
                    ...
                    Gift = 0,
                    Action2 = 0,
                    Mark = finalMark, // <--- Используем вычисленное значение
        ...
        private void CreateProductsGrid()
        {
            try
            {
                ...
                // 1. Главный контейнер (2 строки: шапка + данные)
                ...
                // 4. Настройка колонок (адаптивная логика по разрешению монитора)
                GridLength[] columnLengths;
                ScrollBarVisibility horizontalScrollMode = ScrollBarVisibility.Disabled;

                // Определяем реальное разрешение текущего монитора
                int monitorWidth = 1920; // Значение по умолчанию
                try
                {
                ...
                {
                    // Режим для обычных и широких мониторов: растягиваем пропорционально (Star)
                    columnLengths = new[]
                    {
                ...
                _productsScrollViewer = new ScrollViewer
                {
                    ...
                    HorizontalScrollBarVisibility = horizontalScrollMode,
                    VerticalScrollBarVisibility = ScrollBarVisibility.Auto,
                    Background = Brushes.White,
                    Focusable = true,
        ...
        private void ScrollToSelectedRow(int gridRowIndex)
        {
            try
            {
                if (_productsScrollViewer == null) return;
                ...
                // Логика видимости (упрощенная)
                double viewportTop = _productsScrollViewer.Offset.Y;
                double viewportBottom = viewportTop + _productsScrollViewer.Viewport.Height;
                bool isVisible = (targetPosition >= viewportTop) && (targetPosition + rowHeight <= viewportBottom);
            ...
            try
            {
                ...
                else if (product.Action != 0 || product.Gift != 0)
                {
                    ...
                    else
                    {
                        ...
                        // На всякий случай, если акционная цена пришла без округления, округляем её здесь
                        product.PriceAtDiscount = Math.Round(product.PriceAtDiscount, 2, MidpointRounding.AwayFromZero);
                    }
                }
        ...
        private void RefreshProductsGrid()
        {
            try
            {
                ...
                // Полностью очищаем сетку данных (заголовок трогать не надо, он отдельно)
                _productsTableGrid.Children.Clear();
                _productsTableGrid.RowDefinitions.Clear();
    ...
    bool isFractional)
        {
            ...
            var buttonContainer = new Border
            {
                ...
                Child = CreateButtonPanelStyled(numericUpDown, inputBorder, dialog, isFractional)
            };
            stackPanel.Children.Add(buttonContainer);
        ...
        private TextBox? _currentInnerTextBox;
        private bool _isProcessingTextChange = false;
        private decimal _currentMinValue;
        private bool _currentIsFractional;

        private StackPanel CreateButtonPanelStyled(
            NumericUpDown numericUpDown,
            Border inputBorder,
            Window dialog,
            bool isFractional)
        {
            Console.WriteLine($"=== CreateButtonPanelStyled: isFractional={isFractional} ===");
            ...
            //numericUpDown.ValueChanged += (s, e) => OnNumericUpDownValueChanged(s, e, inputBorder, okButton);

            okButton.Click += (s, e) => OnOkButtonClick(s, e, numericUpDown, inputBorder, dialog);
        ...
        private void OnOkButtonClick(object? sender, RoutedEventArgs e, NumericUpDown numericUpDown, Border inputBorder, Window dialog)
        {
            ...
            {
                Console.WriteLine($"  Closing dialog with value: {Convert.ToDouble(val)}");
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> ItcPrinted()
        {
            NpgsqlConnection? conn = null;
            NpgsqlCommand? command = null;
            bool result = true;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> ItcPrintedP()
        {
            NpgsqlConnection conn = null;
            NpgsqlCommand command = null;
            bool result = true;
