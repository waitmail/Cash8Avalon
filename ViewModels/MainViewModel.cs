...
//        // Метод придется сделать async
//        private async Task<T> OpenWindow<T>(string operationName) where T : Window, new()
//        {
//            try
//            {
//                StatusMessage = $"Открытие {operationName}...";
...
//                var window = new T();
//                _openWindows[typeof(T)] = window;
//                window.ShowInTaskbar = true; // Показываем в панели задач
...
//                if (mainWindow != null)
//                {
//                    // ✅ ShowDialog автоматически устанавливает Owner и держит окно поверх!
//                    await window.ShowDialog(mainWindow);
//                }
//                else
//                {
//                    window.Show(); // Fallback, если главное окно недоступно
//                }
...
namespace Cash8Avalon.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        public MainViewModel()
        {
            // ✅ ИСПОЛЬЗУЕМ AsyncRelayCommand для окон, которые открываются через ShowDialog
            OpenConstantsCommand = new AsyncRelayCommand(OpenConstants, () => CanOpenWindow<Constants>());
            OpenFiscalPrinterCommand = new AsyncRelayCommand(OpenFiscalPrinter, () => CanOpenWindow<FPTK22>());
            OpenInternetDataLoadCommand = new AsyncRelayCommand(OpenInternetDataLoad, () => CanOpenWindow<LoadDataWebService>());
            OpenSettingConnectCommand = new AsyncRelayCommand(OpenSettingConnect, () => CanOpenWindow<SettingConnect>());
            OpenProgramInfoCommand = new AsyncRelayCommand(OpenProgramInfo, () => CanOpenWindow<ProgramInfo>());
            OpenLoadProgramFromInternetCommand = new AsyncRelayCommand(OpenLoadProgramFromInternet, () => CanOpenWindow<LoadProgramFromInternet>());

            // Синхронные команды остаются с обычным RelayCommand
            ExitCommand = new RelayCommand(ExitApplication);
            OpenReceiptsCommand = new RelayCommand(OpenReceipts);
            OpenCashChecksCommand = new RelayCommand(OpenCashChecks);
            CloseContentCommand = new RelayCommand(CloseCurrentContent);
        ...
        // ✅ МЕТОД СТАЛ АСИНХРОННЫМ И ИСПОЛЬЗУЕТ ShowDialog
        private async Task<T> OpenWindow<T>(string operationName) where T : Window, new()
        {
            try
            {
                ...
                // ✅ БЛОКИРУЕМ КНОПКИ МЕНЮ СРАЗУ (чтобы не нажали дважды)
                RefreshAllCommands();
                ...
                StatusMessage = $"{operationName} закрыто";
                return window;
            }
            catch (Exception ex)
            {
        ...
        private void RefreshAllCommands()
        {
            (OpenConstantsCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
            (OpenFiscalPrinterCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
            (OpenInternetDataLoadCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
            (OpenSettingConnectCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
            (OpenProgramInfoCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
            (OpenLoadProgramFromInternetCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();
        }
        ...
        // ✅ Методы стали возвращать Task для совместимости с AsyncRelayCommand
        private async Task OpenConstants() => await OpenWindow<Constants>("Настройки");
        private async Task OpenFiscalPrinter() => await OpenWindow<FPTK22>("Настройка ФП");
        private async Task OpenInternetDataLoad() => await OpenWindow<LoadDataWebService>("загрузки данных");
        private async Task OpenSettingConnect() => await OpenWindow<SettingConnect>("параметры БД");
        private async Task OpenProgramInfo() => await OpenWindow<ProgramInfo>("О программе");
        private async Task OpenLoadProgramFromInternet() => await OpenWindow<LoadProgramFromInternet>("Обновление");

        // === ЛОГИКА ДЛЯ USERCONTROL (остается синхронной) ===
        private void OpenReceipts()
        {
            if (_isReceiptsOpen)
            {
    ...
    // ✅ СТАНДАРТНЫЙ RelayCommand (для синхронных команд)
    public class RelayCommand : ICommand
    {
        private readonly Action _execute;
        private readonly Func<bool>? _canExecute;
    ...
    // ✅ НОВЫЙ AsyncRelayCommand (для асинхронных команд с ShowDialog)
